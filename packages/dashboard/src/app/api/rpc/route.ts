...
export async function POST(request:NextRequest){
  ...
  const helius=process.env.HELIUS_API_KEY ? 'https://'+(mode==='mainnet'?'mainnet':'devnet')+'.helius-rpc.com/?api-key='+encodeURIComponent(process.env.HELIUS_API_KEY) : undefined;
  const rpc=mode==='mainnet'?(process.env.AEGIX_MAINNET_RPC||helius||'https://api.mainnet-beta.solana.com'):(process.env.AEGIX_DEVNET_RPC||helius||'https://api.devnet.solana.com');
